...
class BrowserUseAgent(Agent):
    ...
    async def run(
        ...
        try:
            ...
            for step in range(max_steps):
                ...
                self.state.history.history.append(
                    AgentHistory(
                        ...
                        result=[
                            ActionResult(
                                error=error_message, include_in_memory=True
                            )
                        ],
            ...
            if self.settings.save_playwright_script_path:
                logger.info(
                    "Agent run finished. Attempting to save Playwright script to: "
                    f"{self.settings.save_playwright_script_path}"
                )
                try:
                    keys = (
    ...
    async def _handle_step_error(self, error: Exception) -> list[ActionResult]:
        logger.exception("Agent step error", exc_info=error)
